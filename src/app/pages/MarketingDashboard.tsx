...
import { Megaphone, Star, Send, TrendingUp, Plus, X, Mail, Bell, Edit, Trash2 } from "lucide-react";
import { MOCK_EVENTS, MOCK_CAMPAIGNS, Campaign } from "../mock-data";
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr style={{ borderBottom: "1px solid rgba(249,115,22,0.08)" }}>
                                    {["Campaign", "Type", "Event", "Sent", "Open Rate", "Revenue", "Status", "Actions"].map((h) => (
                                        ...
                                        <tr key={c.id} className="hover:bg-[rgba(249,115,22,0.02)] transition-colors" style={{ borderBottom: "1px solid rgba(249,115,22,0.04)" }}>
                                            ...
                                            <td className="px-4 py-3">
                                                <div className="flex gap-1">
                                                    <button className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: "rgba(96,165,250,0.08)", color: "#2563eb" }} title="Edit"><Edit size={11} /></button>
                                                    <button onClick={() => setCampaigns(prev => prev.filter(x => x.id !== c.id))} className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: "rgba(248,113,113,0.08)", color: "#dc2626" }} title="Delete"><Trash2 size={11} /></button>
                                                </div>
                                            </td>
                                        </tr>
                                    );
