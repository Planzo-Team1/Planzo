...
import { TrendingUp, Users, DollarSign, Star, Activity, Download } from "lucide-react";
...
export function AnalyticsDashboard() {
    ...
    const exportCSV = () => {
        const headers = ["Month", "Tickets Sold", "Revenue", "Attendees", "New Users"];
        const rows = MOCK_ANALYTICS.map(r => [r.month, r.ticketsSold, r.revenue, r.attendees, r.newUsers]);
        const csv = [headers, ...rows].map(r => r.join(",")).join("\n");
        const blob = new Blob([csv], { type: "text/csv" });
        const url = URL.createObjectURL(blob);
        const a = document.createElement("a"); a.href = url; a.download = "planzo_analytics_report.csv"; a.click();
        ...
        <div style={{ paddingTop: 68, minHeight: "100vh", background: "var(--color-bg-base)" }}>
            <div className="px-6 md:px-12 py-8 max-w-[1200px] mx-auto">
                ...
                <div className="flex items-center justify-between mb-8">
                    ...
                    <button onClick={exportCSV} className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold transition-all hover:opacity-90"
