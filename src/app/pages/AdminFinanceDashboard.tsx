...
import { DollarSign, TrendingUp, ArrowUpRight, CheckCircle, Clock, AlertCircle, Download, Check } from "lucide-react";
import { MOCK_REVENUE, RevenueRecord } from "../mock-data";
...
export function AdminFinanceDashboard() {
    ...
    const [records, setRecords] = useState<RevenueRecord[]>(MOCK_REVENUE);
    ...
    const exportCSV = () => {
        const headers = ["Event", "Organizer", "Gross", "Commission", "Payout", "Status", "Date"];
        const rows = records.map(r => [r.eventTitle, r.organizer, r.gross, r.commission, r.payout, r.status, r.date]);
        const csv = [headers, ...rows].map(r => r.join(",")).join("\n");
        const blob = new Blob([csv], { type: "text/csv" });
        const url = URL.createObjectURL(blob);
        const a = document.createElement("a"); a.href = url; a.download = "planzo_finance_report.csv"; a.click();
                    ...
                    <div className="flex items-center justify-between p-5 border-b" style={{ borderColor: "rgba(249,115,22,0.08)" }}>
                        ...
                        <div className="flex items-center gap-3">
                            <input
                                ...
                                className="px-3 py-2 rounded-xl text-xs outline-none w-48"
                                style={{ background: "var(--color-bg-raised)", border: "1px solid rgba(249,115,22,0.2)", color: "#1a0a00" }}
                            />
                            <button onClick={exportCSV} className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold transition-all hover:opacity-90"
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr style={{ borderBottom: "1px solid rgba(249,115,22,0.06)" }}>
                                    {["Event", "Organizer", "Gross", "Commission", "Payout", "Date", "Status", "Action"].map((h) => (
                                ...
                                {filtered.map((r) => {
                                    ...
                                    return (
                                        <tr key={r.id} className="hover:bg-[rgba(249,115,22,0.02)] transition-colors" style={{ borderBottom: "1px solid rgba(249,115,22,0.04)" }}>
                                            ...
                                            <td className="px-4 py-3">
                                                {r.status !== "paid" ? (
                                                    <button onClick={() => markAsPaid(r.id)} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-bold transition-all hover:opacity-90"
