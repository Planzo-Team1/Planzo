...
import { LogIn, Eye, EyeOff, Zap, Mail, ArrowLeft, Check } from "lucide-react";
...
export function AuthLogin() {
    ...
    const handleResetRequest = (e: React.FormEvent) => {
        ...
        setTimeout(() => { setResetSent(true); setResetLoading(false); }, 1200);
    };
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div>
                            ...
                            <div className="text-right mt-1">
                                <button type="button" onClick={() => { setShowForgot(true); setResetSent(false); setResetEmail(""); }} className="text-xs" style={{ color: "#f97316", background: "none", border: "none", cursor: "pointer" }}>Forgot password?</button>
                            </div>
                        </div>
                    ...
                    <div className="w-full max-w-sm rounded-2xl p-6" style={{ background: "var(--color-bg-panel)", border: "1px solid rgba(249,115,22,0.25)" }}>
                        {resetSent ? (
                            <div className="text-center py-4">
                                <div className="w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-4" style={{ background: "rgba(249,115,22,0.1)", border: "2px solid #f97316" }}>
                                    <Check size={24} style={{ color: "#f97316" }} />
                                </div>
                                <p className="font-bold mb-1" style={{ color: "#1a0a00" }}>Email Sent!</p>
                                <p className="text-sm mb-4" style={{ color: "#78716c" }}>Check your inbox for the password reset link.</p>
                                <button onClick={() => setShowForgot(false)} className="px-5 py-2 rounded-xl text-sm font-bold" style={{ background: "linear-gradient(135deg,#f97316,#ef4444)", color: "#fff" }}>Back to Login</button>
                            </div>
                        ) : (
                            <>
                                <div className="flex items-center gap-2 mb-4">
                                    <button onClick={() => setShowForgot(false)}><ArrowLeft size={16} style={{ color: "#78716c" }} /></button>
                                    <h2 className="text-base font-bold" style={{ color: "#1a0a00" }}>Recover Password</h2>
                                </div>
                                <p className="text-sm mb-4" style={{ color: "#78716c" }}>Enter your email and we'll send you a reset link.</p>
                                <form onSubmit={handleResetRequest} className="space-y-4">
                                    <div>
                                        <label className="flex items-center gap-1.5 text-xs font-medium mb-1.5" style={{ color: "#92400e" }}><Mail size={11} /> Email Address</label>
                                        <input type="email" required placeholder="you@example.com" value={resetEmail} onChange={(e) => setResetEmail(e.target.value)} className="w-full px-4 py-3 rounded-xl text-sm outline-none" style={{ background: "var(--color-bg-raised)", border: "1px solid rgba(249,115,22,0.2)", color: "#1a0a00" }} />
                                    </div>
                                    <button type="submit" disabled={resetLoading} className="w-full py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2" style={{ background: "linear-gradient(135deg,#f97316,#ef4444)", color: "#fff" }}>
                                        {resetLoading ? <div className="w-4 h-4 border-2 border-[#fff] border-t-transparent rounded-full animate-spin" /> : "Send Reset Link"}
                                    </button>
                                </form>
                            </>
                        )}
